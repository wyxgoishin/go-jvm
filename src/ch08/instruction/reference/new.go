...
func (n *NEW_ARRAY) FetchOperands(cr *utils.ByteCodeReader) {
	n.atype = cr.ReadUint8()
}
...
func (a *ANEW_ARRAY) Execute(frame *rtda.Frame) {
	rtcp := frame.Method().Class().RuntimeConstantPool()
	classRef := rtcp.GetConstant(a.Index).(*heap.ClassRef)
...
func (m *MULTI_ANEW_ARRAY) FetchOperands(cr *utils.ByteCodeReader) {
	m.index = cr.ReadUint16()
	m.dimension = cr.ReadUint8()
}

func (m *MULTI_ANEW_ARRAY) Execute(frame *rtda.Frame) {
	rtcp := frame.Method().Class().RuntimeConstantPool()
	classRef := rtcp.GetConstant(uint(m.index)).(*heap.ClassRef)
	elemClass := classRef.ResolvedClass()
	counts := popAndCheckCount(frame.OperandStack, int8(m.dimension))
